		...
		<div class="karaoke-container">
			...
			<div v-else class="karaoke-screen">
				...
				<div class="staff-container">
					<svg class="staff" viewBox="0 0 1200 200" preserveAspectRatio="xMidYMid meet">
						<line v-for="i in 5" :key="'line-' + i" :y1="40 + (i - 1) * 20" :y2="40 + (i - 1) * 20" x1="0"
							x2="1200" stroke="#e2e8f0" stroke-width="1.5" />

						<g v-for="(nota, idx) in notasVisiveis" :key="'nota-' + idx">
							<circle :cx="nota.x" :cy="nota.y" r="8"
								:fill="nota.acertou === true ? '#48bb78' : nota.acertou === false ? '#f56565' : '#667eea'"
								:stroke="nota.ativa ? '#2d3748' : 'none'" :stroke-width="nota.ativa ? '3' : '0'" />
							<text :x="nota.x" :y="nota.y - 15" text-anchor="middle" font-size="12"
								:fill="nota.ativa ? '#2d3748' : '#718096'">
								{{ nota.nome }}
							</text>
						</g>

						<circle v-if="notaCantada" :cx="600" :cy="notaCantada.y" r="10" fill="#f6ad55" stroke="#dd6b20"
...
export default {
	...
	computed: {
		...
		notasVisiveis() {
			const janela = this.notasMusicais.filter(nota =>
				nota.tempo >= this.tempoAtual - 2 &&
				nota.tempo <= this.tempoAtual + 5
			);

			return janela.map(nota => {
				const deltaT = nota.tempo - this.tempoAtual;
				const x = 600 + (deltaT / 5) * 500;
				const y = this.notaParaY(nota.nome);
				const ativa = Math.abs(deltaT) < 0.3;

				return { ...nota, x, y, ativa };
			});
		}
	},
	methods: {
		...
		async inicializar() {
			...
			try {
				...
				const resp = await api.get(`/musicas/arquivos/${musicaId}`, {
				...
				const [instrBlob, lrcText] = await Promise.all([
				...
				this._audioObjectUrl = URL.createObjectURL(
					new Blob([instrBlob], { type: 'audio/wav' })
				);
		...
		detectarPitch() {
			...
			const detectar = () => {
				...
				if (pitch > -1) {
					const notaNome = this.frequenciaParaNota(pitch);
					this.notaCantada = {
						nome: notaNome,
						y: this.notaParaY(notaNome)
					};
